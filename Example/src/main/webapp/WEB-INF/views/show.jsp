	...
	<ul class="nav nav-tabs">
     ...
     <ul class="nav nav-tabs">
        ...
        <c:forEach items="${listGames}" var="games"> 
			<center><h3>${games.getNameGame()} <small><a href="<c:url value="/show/app/${games.getId()}" /> ">${games.getNameGame()}</a></small></h3></center>
			<center><a href = "<c:url value="/show/app/${games.getId()}" />"><img class = "img" src="${games.getUrl()}" alt="DINO" title="${games.description }" style="-moz-box-shadow:0 0 10px #000; -webkit-box-shadow:0 0 10px #000; box-shadow:0 0 10px #000;" alt="${games.getNameGame()}" width="283" height="398"></a></center> 
			<c:if test="${name == 'admin'}">
